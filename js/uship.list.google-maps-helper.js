...
;(function (root) {
    ...
    var formatAddressString = function formatAddressString(streetAddress, majorMunicipality, StateProvinceLabel, postalCode, CountryLabel) {
        var value = '';
    ...
    var getFormattedAddress = function getFormattedAddress(address) {
    ...
    var getRitchieBrosFormattedAddress = function getRitchieBrosFormattedAddress(address) {
    ...
    var createFormattedAddressFromCurrentLocation = function createFormattedAddressFromCurrentLocation(placeResult) {
    ...
    var RemovePredictionHighlighting = function RemovePredictionHighlighting(suggestion) {
        if (suggestion) {
            jQuery(suggestion).removeClass('tt-highlighted');
        }
    };

    var waitForGooglePlacesToAutoCompleteInvalidInput = function waitForGooglePlacesToAutoCompleteInvalidInput() {
        setTimeout(function () {
            //wait for typeahead autoselection to propogate
        }, 10000);
    };

    root.uship.namespace('list').extend({
        ...
        MakeVisualSelectionPrediction: MakeVisualSelectionPrediction,
