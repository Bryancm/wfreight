...
!function ($) {
    ...
    SmartBanner.prototype = {
        ...
        create: function create() {
            ...
            if (this.options.url) link = this.options.url;else {
                if (this.type == 'android') {
                    link = 'market://details?id=' + this.appId;
                    if (this.options.GooglePlayParams) link = link + '&referrer=' + this.options.GooglePlayParams;
        ...
        show: function show(callback) {
            ...
            if (this.options.layer) {
                ...
                if ($.support.transition) {
                    var transitionCallback = function transitionCallback() {
                        $('html').removeClass('sb-animation');
                        if (callback) {
                            callback();
                        }
                    };
                    $('html').addClass('sb-animation').one($.support.transition.end, transitionCallback).emulateTransitionEnd(this.options.speedIn).animate({ 'margin-top': 82 * this.scale }, this.options.speedIn);
